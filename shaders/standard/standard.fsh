...
if(fragment_mode == 0)
{
  ...
  if(boolean_phase == 2)
  {
      ...
      float back_b_depth = dot(texture2D(boolean_depth_b_sampler, sample_coord + vec2(a_pixel.x/2.0, 0.0)), vec4(1.0, 1.0/255.0, 1.0/(255.0*255.0), 1.0/(255.0*255.0*255.0))) * 300.0;
      float front_b_depth = dot(texture2D(boolean_depth_b_sampler, sample_coord), vec4(1.0, 1.0/255.0, 1.0/(255.0*255.0), 1.0/(255.0*255.0*255.0))) * 300.0;
