...
for(var triangle = 0; triangle < model_size; triangle++)
{
	...
	for(var vertex = 0; vertex < 3; vertex++)
	{
		...
		for(var coord = 0; coord < 3; coord++)
		{
			...
			current_normal[coord] = position/COMPRESSED_NORMAL_POSITION;
			if(position < 0)
				current_normal[coord] = ceil(current_normal[coord])*COMPRESSED_NORMAL_POSITION;
			else
				current_normal[coord] = floor(current_normal[coord])*COMPRESSED_NORMAL_POSITION;

			position -= current_normal[coord];

			current_normal[coord] /= 128*COMPRESSED_NORMAL_POSITION;
