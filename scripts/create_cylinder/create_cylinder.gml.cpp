...
if(abs(argument0) >= COMPRESSED_NORMAL_POSITION) argument0 = COMPRESSED_NORMAL_POSITION - 0.001;
for(var coord = 0; coord < 2; coord++)
{
    var limit = (1 +  !argument2[coord])*COMPRESSED_NORMAL_POSITION;
...
ENCODE_COLOR(encoded_white, argument7[0], argument7[1], argument7[2], argument7[3])
...
var end_angle = 0;
for(end_angle = 2*pi/argument3; ; end_angle += 2*pi/argument3)
{
    if(end_angle > 2*pi*argument4)
        end_angle = 2*pi*argument4;
    ...
    if(end_angle == 2*pi*argument4) break;
}
...
if(argument4 != 1)
{
    ...
    var normals = [0,0];
    normals[0] = [0,1,0];
    normals[1] = [cos(end_angle - pi/2), sin(end_angle - pi/2),0];
